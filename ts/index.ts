...
const playerIcon = document.querySelector(".game__ui-icon-0") as HTMLDivElement;

const cpuIcon = document.querySelector(".game__ui-icon-1") as HTMLDivElement;

const cpuNameEl = document.querySelector(
  ".game__ui-cpu-name"
) as HTMLSpanElement;
...
const choices: [string, string, string] = ["Rock", "Paper", "Scissors"];

const cpuPlayerNames: string[] = [
  "Bob",
  "Emma",
  "Steve",
  "Lauren",
...
const choicesIcons: [string, string, string] = [
...
const randomCpuName = (): string =>
  cpuPlayerNames[Math.floor(Math.random() * cpuPlayerNames.length)];

const cpuName: string = randomCpuName();

cpuNameEl.innerText = `${cpuName}'s`;

const makeChoice = (): string =>
  choices[Math.floor(Math.random() * choices.length)];

const updateUI = (userChoice: string, cpuChoice: string): void => {
  playerIcon.innerHTML =
    choicesIcons[choices.findIndex((val: string) => val === userChoice)];
  cpuIcon.innerHTML =
    choicesIcons[choices.findIndex((val: string) => val === cpuChoice)];
};

const hideUI = (isTrue: boolean = true): void => {
  if (isTrue) {
    gameHeading.classList.add("hide");
    playerIcon.classList.add("scale-down");
    cpuIcon.classList.add("scale-down");
    gameButtons.forEach((btn: HTMLButtonElement) => (btn.disabled = true));
  }
  ...
  if (!isTrue) {
    ...
    gameButtons.forEach((btn: HTMLButtonElement) => (btn.disabled = false));
  }
};
...
const updateScores = (
  ...
  if (!draw) {
    ...
    playerScore.classList.add("move");
    setTimeout(() => {
      playerScore.classList.remove("move");
    }, 600);
    heading.innerHTML = `${winningCondition}`;
    return;
  }
  heading.innerHTML = `${winningCondition}`;
};

const gameResult = (userChoice: string, cpuChoice: string): void => {
...
gameButtonsContainer.addEventListener("click", (e: any) => {
  const userChoice: string = e.target.closest("button")?.dataset?.choice;
  const cpuChoice: string = makeChoice();
  if (userChoice) {
    hideUI();
    setTimeout(() => {
      updateUI(userChoice, cpuChoice);
